 ...
 * description and phases) and offers: Run once / View script / Open in editor /
 * Deny. An inline script is a new script every time, so it never skips the
 * dialog. Deny throws a clear error the model can relay.
 */
...
export const approveLaunch: ApproveLaunch = async (plan, ctx, deps) => {
  ...
  const options = [RUN_ONCE, VIEW, OPEN, DENY];
