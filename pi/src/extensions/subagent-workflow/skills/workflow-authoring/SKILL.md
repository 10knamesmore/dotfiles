...
- Start with `mid`: code exploration, repository navigation, search, evidence
  gathering, initial triage, quick feedback, and well-scoped implementation or
  edits. Large or unfamiliar codebases are still suitable for `mid` exploration.
  Choose `high` for sustained reasoning across interacting behaviors and
  tradeoffs; `max` for the hardest root-cause analysis, cross-module design with
  major uncertainty, or consequential decisions requiring the deepest judgment.
  Gathering evidence for a difficult task still belongs on `mid`; reserve stronger
  tiers for the difficult analysis or decision itself. File count, prompt length,
