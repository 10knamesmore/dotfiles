...
export type TierSubagentSpec = Omit<SubagentSpec, "model" | "modelTier"> & {
  model: ModelTier;
};

/** Explicit overrides an orchestrator may pick. */
const THINKING_LEVEL_OVERRIDES = ["medium", "xhigh", "max"] as const;

/**
 * Omitting the field inherits the parent's level, and follow-ups replay the
...
export const SubagentPromptSchema = Type.String({
  minLength: 1,
  description:
    "Self-contained task for the child. The child does not receive the parent conversation; include every fact and output requirement it needs.",
});

export const MODEL_DESCRIPTION =
  "Required model tier: max, high, or mid. Use the current model-tier mappings and guidance in the system prompt. Provider/model-id strings are not accepted. No default or parent-model inheritance.";
...
export const PublicSubagentOptionFields = {
  ...
  tools: Type.Optional(
    Type.Array(Type.String(), {
      description:
        "Tool-name allowlist. Normally omit to use tools discovered for the child cwd; requesting an unavailable tool fails that child.",
    }),
  ),
  ...
  cwd: Type.Optional(
    Type.String({
      description:
        "Child working directory. Omit to inherit the parent cwd; extension and tool discovery follows the child cwd.",
    }),
  ),
  label: Type.Optional(
    Type.String({
      description: "Short display label for status and recovery output.",
    }),
  ),
};
