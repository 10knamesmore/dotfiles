...
export interface ChildRow {
  ...
  activity?: string;
  resultLine?: string;
  /** Authoritative terminal output; transcript browsing must not infer it from the last message. */
  result?: Pick<SubagentResult, "status" | "text" | "structured">;
  error?: string;
  sessionFile?: string;
...
export function foldRunProjection(projection: RunProjection, event: RunProjectionEvent): void {
  ...
  if (event.type === "status") {
    ...
    child.result = {
      ...
      ...(Object.hasOwn(event.result, "structured") ? { structured: event.result.structured } : {}),
    };
