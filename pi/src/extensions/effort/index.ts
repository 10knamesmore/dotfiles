...
export function registerEffortAlias(pi: ExtensionAPI): void {
  pi.registerCommand("effort", {
    ...
    getArgumentCompletions: (prefix) => {
      ...
      const matches = ALL_LEVELS.filter((level) => level.startsWith(normalized));
    ...
    handler: async (rawArgs, ctx) => {
      ...
      if (!level) {
        ctx.ui.notify(`Unknown thinking level "${rawArgs.trim()}". Available levels: ${levels.join(", ")}.`, "error");
        return;
      }
...
function applyLevel(pi: ExtensionAPI, ctx: ExtensionCommandContext, level: ModelThinkingLevel): void {
 ...
 */
function pickLevel(pi: ExtensionAPI, ctx: ExtensionCommandContext, levels: ModelThinkingLevel[]): Promise<void> {
  return ctx.ui.custom<void>((_tui, _theme, _keybindings, done) => {
    const select = (level: ModelThinkingLevel) => {
      applyLevel(pi, ctx, level);
      done();
    };
    return new ThinkingSelectorComponent(pi.getThinkingLevel(), levels, select, () => done(), select);
  });
}
