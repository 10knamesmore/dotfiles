...
`python_pi` 在一个活跃 Pi 会话中复用同一个 Python 进程。名字避开 ChatGPT Codex 后端保留的 `python`（网关会在上行把同名工具改名）。会话启动时通过 `uv run --project … --locked` 准备环境并读取实际 Python 版本及已安装的第三方发行包版本，注入工具描述。包列表包含传递依赖，按名称排序。持久 worker 在首次调用时启动，后续调用直接发送代码。
