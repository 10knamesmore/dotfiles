...
function toolStatusLabel(block: ToolBlock): string {
  switch (block.status) {
    ...
    case "waiting": return "◌ awaiting result";
    case "unavailable": return "? result not loaded";
  }
}

function toolColor(block: ToolBlock): string {
  return block.status === "failed" ? "error" : block.status === "completed" ? "success" : block.status === "waiting" ? "accent" : "muted";
}
...
function blockColor(block: TranscriptBlock): string {
  switch (block.kind) {
    case "thinking": return "thinkingMedium";
    case "tool": return block.status === "failed" ? "error" : "toolTitle";
    case "task": return "accent";
    case "notice": return "warning";
    case "output": return block.final ? "success" : "text";
  }
}
...
function renderBlock(
  ...
  if (block.kind === "tool") {
    ...
    if (expanded && !state.outputOnly) {
      ...
      else body.push(theme.fg("muted", block.status === "waiting" ? "Waiting for the tool to return." : "Result unavailable in the loaded history."));
    }
  } else if (block.kind === "thinking") {
    heading += theme.fg("muted", `  ${expanded ? "expanded" : "folded"}${block.body.elided ? " · partial history" : ""}`);
    ...
    heading += theme.fg(block.final ? "success" : "muted", block.final ? `  Final result${block.structured ? " · JSON" : ""}` : "  Message");
...
export function renderTranscript(
  ...
  const visible = blocks.filter((block) => !state.outputOnly || block.kind === "output" || block.kind === "notice" || (block.kind === "tool" && block.status === "failed"));
  ...
  for (let index = visible.length - 1; index >= 0; index -= 1) {
    if (remaining < 4) { omitted = true; break; }
  ...
  if (layout.lines.length === 0) layout.lines.push(theme.fg("muted", state.outputOnly ? "No output yet." : "No transcript yet."));
