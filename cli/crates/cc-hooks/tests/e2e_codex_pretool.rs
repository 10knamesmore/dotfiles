//! 端到端：起真实 `agent-hook codex-pretool`，验证 Codex PreToolUse 协议与降级语义。
...
/// 仓库中的真实生产规则；Codex adapter 必须与 Claude 共用这份语义。
...
/// 运行 Codex adapter 并返回 stdout。
fn run(
    rules_path: &std::path::Path,
    stdin_json: &str,
) -> Result<String, Box<dyn std::error::Error>> {
    let output = Command::cargo_bin("agent-hook")?
        .args(["codex-pretool", "--rules"])
        .arg(rules_path)
    ...
    assert!(
        ...
        "Codex hook 必须保持 fail-open exit 0"
    );
    Ok(String::from_utf8(output.stdout)?)
}

/// 提取 Codex PreToolUse 的 permissionDecision 与理由。
...
fn ask_degrades_to_explicit_deny() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        reason.contains("Codex PreToolUse 暂不支持 ask"),
...
#[test]
fn production_rules_are_compatible_with_codex_adapter() -> Result<(), Box<dyn std::error::Error>> {
    let rules = rules_file(PROD_RULES)?;
    let deny = run(rules.path(), &bash_envelope("grep needle file.txt"))?;
    let ask = run(rules.path(), &bash_envelope("git reset --hard HEAD~1"))?;
    assert_eq!(
        decision(&deny)?.map(|value| value.0),
        Some("deny".to_owned())
    );
    let Some((actual, reason)) = decision(&ask)? else {
        return Err("生产 ask 规则不能在 Codex 侧静默放行".into());
    };
    assert_eq!(actual, "deny");
    assert!(reason.contains("Codex PreToolUse 暂不支持 ask"));
    Ok(())
}
