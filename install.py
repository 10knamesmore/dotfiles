    ...
    def _setup_zshrc_stub(self, zshrc: Path) -> None:
        ...
        if zshrc.is_symlink():
            ...
            if self.ZSHRC_MARKER in first_line:
                self.info("~/.zshrc 已由 dotfiles 管理，跳过（保留软件追加内容）")
                return
            else:
                # 用户自有的 .zshrc，备份后替换
                self.backup_file(zshrc)

        self.action(f"创建 ~/.zshrc stub: {zshrc}")
        if not self.dry_run:
            zshrc.write_text(stub_content)
