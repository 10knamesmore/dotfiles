...
PanelWindow {
    ...
    function ensureAppsLoaded() {
        ...
        for (let i = 0; i < apps.length; i++) {
            if (!apps[i].noDisplay) list.push(apps[i])
        }
        allApps = list
        appsLoaded = true
    }
    ...
    function fuzzyScore(q, text) {
        ...
        // 完全前缀匹配加大分
        if (lower.startsWith(ql)) return 1000 + (100 - text.length)
            ...
            ListView {
                ...
                delegate: Rectangle {
                    ...
                    color: index === root.selectedIndex
                        ? Colors.surface1
                        : appHover.containsMouse ? Qt.rgba(Colors.surface1.r, Colors.surface1.g, Colors.surface1.b, 0.5) : "transparent"
