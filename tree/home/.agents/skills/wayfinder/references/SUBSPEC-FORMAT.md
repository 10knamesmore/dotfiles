...
- `status`：`draft`、`ready`、`in-progress`、`resolved` 或 `cancelled`。`resolved` 还必须满足 Wayfinder 的 commit gate。
...
- 只有写完 `Resolution` 与 `Evidence`、完成实际验证，并且用户明确指定本轮相关改动需要 commit 且该 commit 已实际创建后，才能改为 `resolved`。
- 如果 commit gate 未满足，可以先写入 `Resolution` 与 `Evidence`，但必须保持 `in-progress`，不得把该 Subspec 当作 resolved dependency。
