...
- 禁止过度工程， 过度防御， 过度校验, 任何时候没用户允许, **只写不加任何防御的业务代码**, 任何其他的防御在和用户讨论并确认前禁止添加
- **禁止** 代码/注释/面对用户的文本 等地方使用日文引号
- 声明"完成 / 修好 / 通过"前先实际验证（跑命令看输出）；没验证的步骤如实说，失败不粉饰
- 任何一个功能,先翻项目里已有的依赖能做什么, 优先用成熟的、有人维护的库。没有明确理由别自己写轮子。先看成熟产品怎么解决同一个问题，用已验证的模式，别从零发明。
...
- GitHub 操作一律 `gh` CLI，深入研究仓库 clone 到 documents/repo
