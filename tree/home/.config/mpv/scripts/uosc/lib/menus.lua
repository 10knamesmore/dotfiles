...
function create_self_updating_menu_opener(opts)
	return function()
		...
		-- items 和 active_index 在 handle_prop_change 回调里设置；由于添加 property
		-- observer 会立即触发其 handler，我们就直接让那次触发来初始化条目。
		menu = Menu:open({
			type = opts.type,
			title = opts.title,
...
-- 打开一个文件导航菜单，列出 `directory_path` 内的条目。
...
function open_file_navigation_menu(directory_path, handle_activate, opts)
	...
	---@param path string 可以是目录路径，或特殊字符串 `'{drives}'`（用于获取 windows 盘符条目）。
	---@param selected_path? string 把路径与之匹配的条目标记为 active。
	...
	local function serialize_items(path, selected_path)
		...
		-- 若不在根目录，则预先放入指向上级目录的选择项。
		-- 每个条目的 value 是它指向的序列化路径表。
		local items = {}
	...
	---@param event MenuEventActivate
	---@param only_if_dir? boolean 仅当条目是目录时才激活。
...
do
	...
	-- 返回 `input.conf` 中所有相关的 binding，即使它们被覆盖
	-- （同一按键后面又被绑到别处）或没有按键（uosc 菜单条目）。
...
function get_keybinds_items()
	local items = {}
	-- uosc 和 mpv-menu-plugin 中没有按键的绑定
...
function open_stream_quality_menu()
	...
	menu = Menu:open({type = 'stream-quality', title = t('Stream quality'), items = items}, function(event)
		if event.type == 'activate' then
			...
			-- 重新加载视频以应用新格式
			-- 取自 https://github.com/jgreco/mpv-youtube-quality
			-- 而后者又取自 https://github.com/4e6/mpv-reload/
			local duration = mp.get_property_native('duration')
			local time_pos = mp.get_property('time-pos')

			mp.command('playlist-play-index current')

			-- 尝试区分直播流与预录的 VOD。VOD 的 duration
			-- property 非零。重新加载 VOD 时，为保持当前时间位置，
			-- 应提供相对起点的偏移量。直播流没有固定起点，
			-- 比较合理的做法是从当前的 'live' 位置重新加载。
			-- 这就是重新加载直播流时不传偏移量的原因。
...
function open_subtitle_downloader()
	...
	-- 检查是否有 error 或 data 无效。若是，则报告错误、
	-- 更新菜单以提示用户，并返回 true。
	---@param error string|nil
	---@param data any
	---@param check_is_valid? fun(data: any):boolean
	---@return boolean abort 是否应中止后续的响应处理。
	...
	handle_search = function(query, page)
		...
		args[#args + 1] = table.concat(table_keys(create_set(languages)), ',') -- 去重，处理 `en,eng,en` 这类情况
