...
---@param opacity number|{primary?: number; border?: number, shadow?: number, main?: number} 所有元素的不透明度。
---@param fraction? number 可选,用此比例系数调整上述不透明度。
---@return string|nil
...
-- 时间戳,每个数字的定位都当作它被替换成 0 来计算(以保持宽度稳定)
---@param x number
---@param y number
---@param align number
...
---@param bx number
---@param by number
---@param points number[] 归一化点(0–1)的扁平表:起点,后接各段条目 cp1x, cp1y, cp2x, cp2y, px, py, ...
