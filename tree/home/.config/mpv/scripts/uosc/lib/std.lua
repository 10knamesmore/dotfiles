...
---@param rgba string `rrggbb` 或 `rrggbbaa` 十六进制字符串。
...
---@param from? number 搜索的起始位置，默认为 `1`。
---@param to? number 搜索的结束位置，默认为 `#itable`。
...
-- 把 `source` 中不在 `props` 集合里的属性赋值给 `target`。
---@generic T: table<any, any>
---@param target T
---@param source T
...
-- 相当于 `table_assign({}, input)`，但不会丢失类型 :(
---@generic T: table<any, any>
---@param input T
---@return T
function table_copy(input) return table_assign({}, input) end

-- 把数组表的值转换成 `table<value, true>` 映射。
---@param values any[]
...
---@param key string 按键，或 `modifiers+key` 形式的组合。若其中包含修饰键，则忽略 `modifiers` 参数。
