...
config = {
	...
	types = {
		...
		load = {}, -- 由下方的 update_load_types() 填充
	},
...
function update_config()
	...
	-- 添加带有强制可见状态的 `{element}_persistency` 配置属性（例如：`{paused = true}`）
...
state = {
	...
	history = {}, -- 最近播放文件的历史记录，以完整路径存储
	time = nil, -- 当前媒体播放时间
	speed = 1,
	---@type number|nil
	duration = nil, -- 当前媒体时长
	max_seconds = nil, -- 时间轴上时间预期达到的最大秒数，已计入播放速度
	time_human = nil, -- 人类可读格式的当前播放时间
	destination_time_human = nil, -- 取决于 options.destination_time
	...
	is_idle = false,
	is_video = false,
	is_audio = false, -- 文件为纯音频（mp3 等）时为 true
	is_image = false,
	is_stream = false,
	has_image = false,
	has_audio = false,
...
external = {} -- 由外部脚本设置的属性
key_binding_overwrites = {} -- key_binding:mpv_command 的映射表
...
mp.observe_property('demuxer-cache-state', 'native', function(prop, cache_state)
	...
	-- 将已缓存区间反转为未缓存区间，因为我们渲染的正是未缓存的部分
	...
	for _, range in ipairs(inverted_ranges) do
		if last_range and last_range[2] + 0.5 > range[1] then -- 合并相邻区间
			last_range[2] = range[2]
		else
			if range[2] - range[1] > 0.5 then -- 跳过过短的区间
...
-- 添加一个会遵循 `key_binding_overwrites` 表中所设重定向的按键绑定。
---@param name string
...
bind_command('paste-to-playlist', function()
	-- 若没有加载任何文件，则使用 `paste-to-open`，它既会打开文件又会加入播放列表
...
-- 元素管理器
-- 根据用户+脚本的 disabled_elements 配置来处理元素的创建与销毁。
Manager = {
	-- 受管理的可禁用元素 ID
	_ids = itable_join(table_keys(constructors), {'idle_indicator', 'audio_indicator'}),
	---@type table<string, string[]> 客户端到其所禁用元素 id 列表的映射
	_disabled_by = {},
	---@type table<string, boolean>
	disabled = {},
}

-- 设置某客户端及其希望禁用的元素。要撤销只需对同一个 `client` 传入空的 `element_ids`。
---@param client string
