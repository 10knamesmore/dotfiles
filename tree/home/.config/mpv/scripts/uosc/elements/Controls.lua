...
-- sizing(尺寸策略):
--   static - 收缩,对可用空间有最高优先级,空间不足时直接消失
--   dynamic - 收缩以给 static 元素让位,直到缩到 ratio_min,再不够就消失
--   gap - 没有剩余空间时收缩
--   space - 扩展填满可用空间,按需收缩
-- scale - `options.controls_size` 的缩放系数。
-- ratio - static 或 dynamic 元素的宽高比。
-- ratio_min 'dynamic' 尺寸元素的最小比例。
...
function Controls:init()
	...
	---@type ControlItem[] 从 `options.controls` 序列化出的所有控制元素。
	self.controls = {}
	---@type ControlItem[] 仅包含匹配当前 disposition 的控件。
	self.layout = {}
...
function Controls:init_options()
	...
	for i, item in ipairs(items) do
		...
		-- 把 disposition 序列化成「若干组 AND 条件」的 OR 组
		---@type {[string]: boolean}[]
		local dispositions = {}
		---@type string[]
...
function Controls:reflow()
	-- 只把未隐藏且匹配当前 disposition 的项填入 layout
	self.layout = {}
	for _, control in ipairs(self.controls) do
		local matches = false
		local conditions_num = 0

		-- 对照「若干组 AND 条件」的 OR 组进行检查
...
---@param badge string
---@param element Element 一个支持 `badge` 属性的元素。
...
function Controls:update_dimensions()
	...
	-- 计算 statics_width、min_content_width,并统计 space 与 gap 的数量
	for c, control in ipairs(self.layout) do
