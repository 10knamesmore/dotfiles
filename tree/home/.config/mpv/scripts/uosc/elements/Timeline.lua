...
function Timeline:init()
	...
	self.min_progress_size = 0 -- 用于 `flash-progress`
	self.font_size = 0
	self.top_border = 0
	self.line_width = 0
...
function Timeline:render()
	...
	-- 文本 opacity 在即将溢出前,或在到达 progress_size 前迅速降到 0
	...
	local foreground_coordinates = round(fax) .. ',' .. fay .. ',' .. round(fbx) .. ',' .. fby -- 用于 clipping

	-- 时间从 0.5 像素处开始
	local time_ax = bax + 0.5
	local time_width = self.width - line_width - 1

	-- time to x: 计算 x 坐标,使其永远不会落在 line 内部
	...
	ass:draw_start()
	ass:rect_cw(bax, bay, fax, bby) --进度左侧
	ass:rect_cw(fbx, bay, bbx, bby) --进度右侧
	ass:rect_cw(fax, bay, fbx, fay) --进度上方
	ass:draw_stop()
	...
	-- 根据 'timeline_style' 调整绘制顺序,以保持 heatmap 可见
	if is_line then
		draw_heatmap()
		draw_progress()
	else
