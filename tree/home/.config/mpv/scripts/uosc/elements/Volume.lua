...
function VolumeSlider:init(props)
	...
	self.pressed = false
	self.nudge_y = 0 -- 音量溢出 100 时的垂直位置
	self.nudge_size = 0
...
function VolumeSlider:render()
	...
	-- 在指定位置绘制一个带 nudge（凸起）的矩形
	---@param p number 距滑块边缘的内边距。
	---@param r number 圆角半径。
	---@param cy? number 从底部裁剪路径所用的 y 坐标。
	function create_nudged_path(p, r, cy)
		...
		if cy > by - d then
			...
			local xbd = (r - subtracted_radius * 1.35) -- x 方向贝塞尔增量
...
function Volume:update_dimensions()
	...
	self.enabled = height > self.size * 2 -- 太小则不渲染
