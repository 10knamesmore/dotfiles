...
local options = {
    ...
    -- 缩略图生成的最大尺寸（像素，按比例缩小以适配）
    -- 启用 hidpi 时这些值会被相应缩放
    max_height = 200,
    max_width = 200,

    -- 缩略图显示尺寸的缩放系数（需要 mpv 0.38+）
    -- 注意这种方式的画质低于直接增大 max_height 和 max_width
    scale_factor = 1,

    -- 应用 tone-mapping，设为 no 则禁用
    tone_mapping = "auto",

    -- Overlay id
    overlay_id = 42,

    -- 文件加载时即启动缩略图进程，以更快得到首批缩略图
    spawn_first = false,

    -- 闲置指定秒数后关闭缩略图进程，0 表示禁用
    ...
    -- 仅 Windows：使用原生 Windows API 写入管道（需要 LuaJIT）
    direct_io = false,
...
if options.direct_io then
    ...
    if ffi_loaded then
        winapi = {
            ...
            -- 不关心 WriteFile 实际写了多少字节，所以只分配一处用来存放结果
            _lpNumberOfBytesWritten = ffi.new("unsigned long[1]"),
        }
        -- 缓存 run() 中要用的 flags，避免重复调用 bor()
    ...
    subprocess(args, true,
        function(success, result)
            if spawn_waiting and (success == false or (result.status ~= 0 and result.status ~= -2)) then
                ...
                if not spawn_working then -- 提示用户进行必要的配置
                    if options.mpv_path == "mpv" then
                        if properties["current-vo"] == "libmpv" then
                            if options.mpv_path == mpv_path then -- 尝试定位 ImPlay
                                mpv_path = "ImPlay"
                                spawn(time)
                            else -- ImPlay 不在 path 中
...
local function move_file(from, to)
    ...
    -- 移动文件，因为 overlay-add 读取它的同时它可能被覆写，从而让播放器崩溃
    os.rename(from, to)
end
...
local function check_new_thumb()
    -- 从属进程可能在我们检查文件存在性和有效性之后、真正移动文件之前
    -- 开始写入该文件，所以先把文件移到临时位置再做有效性检查，以确保
    -- 一切保持一致，并避免有效的缩略图被无效的覆盖
