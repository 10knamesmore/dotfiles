...
--- LSP server 配置项（`opts.servers` 的值类型），在 vim.lsp.Config 基础上加本配置自用字段
---@class LspServerConfig: vim.lsp.Config
---@field enabled? boolean 设为 false 时禁用该 server
---@field mason? boolean 设为 false 时不交给 mason 安装
---@field keys? LspKeysSpec[] 附着到该 server buffer 的按键

--- LSP 按键声明（lazy.nvim keys 规范 + LSP 过滤字段）
---@class LspKeysSpec: LazyKeysSpec
---@field has? string|string[] 要求客户端支持的 LSP method（无 `/` 时自动补 textDocument/ 前缀）
---@field enabled? boolean|fun():boolean 动态开关

--- Keys.resolve 之后的 LSP 按键对象
---@class LspKeys: LazyKeys
