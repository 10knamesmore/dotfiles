...
`tree/home/.agents/` 是本地 AI 资产的唯一真相源：`skills/` 分发到 `~/.claude/skills`、`~/.codex/skills`（逐 skill 链接），`claude/agents|commands/` 分发到 `~/.claude/` 对应目录。接入新工具 = `dots.lua` 的 `to` 列表加一行 + `dots sync`。

Claude Code 还有一套自建的 PreToolUse 守卫引擎（`cc-hook`，Rust，源在 `cli/crates/cc-hooks/`）：规则表 `pretool.toml` 拦高危命令、做工具偏好重定向。资产地图、规则词汇、部署链与测试见 [AI_TOOLING.md](docs/AI_TOOLING.md)。
...
- AI 工具：Claude / Codex skills 与 agents、opencode、cc-hook 守卫
...
- [AI_TOOLING.md](docs/AI_TOOLING.md) — AI 工具链配置：资产地图、cc-hook 守卫引擎、规则表词汇、部署链与测试
- [MANUAL_SETUP.md](docs/MANUAL_SETUP.md) — 需要 root 手动安装的部分（udev / systemd 键盘 inhibit）
