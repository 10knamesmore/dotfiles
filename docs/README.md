...
`tree/home/.agent/` 是本地 AI 资产的唯一真相源——skills 是公开标准、不专属某个工具，所以源住中立目录，Claude 和 codex/copilot 一样是 `distribute()` 的订阅者：

- `skills/` → 镜像到 `~/.agent/skills`；分发到 `~/.claude/skills`、`~/.codex/skills`、`~/.copilot/skills`（逐 skill 链接）
- `claude/agents/` → Claude 专属格式（其他工具格式未知，按工具命名空间隔离）；分发到 `~/.claude/agents`

落点保持真实目录，本地（不入库的）skill/agent 可与受管链接共存。接入新工具 = `to` 列表加一行 + `dots sync`。
