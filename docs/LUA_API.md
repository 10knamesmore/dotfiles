...
│       └── basic.conf
└── .claude/                              ~/.claude/                                    （.claude 也是层根一级目录
    └── skills/                           └── skills ─────────→ tree/home/.claude/skills   → 真实目录、下钻逐子项链）
```

三条启发式（`cli/crates/dots-core/src/layer.rs:127-131`）：

1. **文件**直接链（①）
2. **层根的一级目录**（`.config/`、`.claude/`）是「容器」：保持真实目录，下钻逐子项链（②）
3. **二级及更深的目录**整目录一根链（③）
...
│   夹着链接执行（完整时间线见 on() 一节）        │
│ · vars/link/dots.json/dots.file/dots.run_once  │
│   等写原语此时才被注入，仅在闭包内可用          │
└──────────────────────────────────────────────┘
```
...
声明期的 Lua 环境移除了 `io`、`os`、`require`、`dofile`、`loadfile`、`load`、`loadstring`、`package`（`eval.rs:60-75`）。`dots.lua` 里写 `os.getenv(...)` 会直接报错——需要环境信息用只读的 `dots.host` / `dots.os` / `dots.home`（见 §3）。
...
├── rust-tdd/          ┌──────────  ~/.claude/skills ←──── 主落点：常规镜像（不归 distribute 管）
├── quickshell/        │
├── gh-cli/            │            ~/.codex/skills/            （mode=children：真实目录，逐 skill 链）
└── …                  ├──────────  ├── rust-tdd ────→ tree/home/.claude/skills/rust-tdd
                       │            ├── quickshell ──→ tree/home/.claude/skills/quickshell
                       │            └── …
                       │
                       └──────────  ~/.copilot/skills/
                                    ├── rust-tdd ────→ tree/home/.claude/skills/rust-tdd
...
为什么落点用 `children` 而不是 `dir`：codex 会往 skills 目录写 `.system/` 之类的运行时产物——逐 skill 链让垃圾留在 `~/.codex/skills/` 真实目录里，不污染源。
...
distribute("skills", {
  src = "tree/home/.claude/skills",
  to = { "~/.codex/skills", "~/.copilot/skills", "~/.newtool/skills" },  -- ← 加一行即可
  mode = "children",
})
```
